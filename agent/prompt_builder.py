...
KANBAN_GUIDANCE = (
    ...
    "\n"
    "## Protocol requirement (MUST — P246/MOL-2011)\n"
    "\n"
    "Your LAST tool call before your session ends MUST be one of:\n"
    "- `kanban_complete(summary=..., metadata=...)` — task finished successfully\n"
    "- `kanban_block(reason=\"...\")` — unrecoverable blocker, needs human decision\n"
    "\n"
    "Exiting without calling either is a **protocol violation**. The dispatcher "
    "will auto-block your task as `crashed: protocol violation`, and your work "
    "may be lost because downstream workers cannot see an unclosed task.\n"
    "\n"
    "### Partial success\n"
    "If you completed SOME of the work but not all (e.g. cherry-picked 11 of 16 "
    "commits, 5 were empty/already-covered):\n"
    "1. Call `kanban_complete` with a summary of what you DID accomplish (be specific — "
    "list SHAs, files, counts).\n"
    "2. In `metadata`, include `{\"partial\": true, \"remaining\": \"<what still needs "
    "to be done>\"}`.\n"
    "3. If the remaining work can be a follow-up task, call `kanban_create` to file it. "
    "If it needs a human decision, call `kanban_block` instead.\n"
    "\n"
    "### Cherry-pick tasks\n"
    "After cherry-picking upstream commits:\n"
    "1. Call `kanban_heartbeat` with each batch of commits that landed (e.g. "
    "`\"cherry-picked: fcd9011f8 OAuth PKCE, cd9470f41 DeepSeekProfile\"`).\n"
    "2. In your final `kanban_complete`, include the full list of applied SHAs in "
    "`metadata.changed_files` and any conflicts resolved in `metadata.decisions`.\n"
    "3. If conflicts prevent completion, call `kanban_block` with the specific file "
    "and conflict details — do NOT exit silently."
)
